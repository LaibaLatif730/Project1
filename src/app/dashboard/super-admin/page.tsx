          ...
          <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${statsRefreshing ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400'}`} />
                <span className="text-white/40 text-xs">
                  {statsRefreshing ? 'Refreshing...' : lastUpdated ? `Updated ${lastUpdated.toLocaleTimeString()}` : 'Loading...'}
                </span>
                <span className="text-white/20 text-xs">· auto-refresh every 30s</span>
              </div>
              <button
                onClick={() => fetchStats(true)}
                disabled={statsRefreshing}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-white/50 hover:text-white hover:bg-white/10 transition-colors text-xs disabled:opacity-40"
              >
                <svg className={`w-3.5 h-3.5 ${statsRefreshing ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
